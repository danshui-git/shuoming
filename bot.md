      ...
      run: |
        if [[ "${BOT}" == "TRUE" ]]; then
          if [[ "${SERVERCHAN_SCKEY}" == "true" ]]; then
            ...
            您使用【${{matrix.target}}】文件夹编译的[${{ env.NAME2 }}-${{ env.NAME1 }}]固件(${{env.CangKu}}仓库的#${{env.Run_number}}号)顺利编译完成了！💐" "https://api.telegram.org/bot${{ secrets.TELEGRAM_BOT_TOKEN }}/sendMessage"
          fi
        fi
