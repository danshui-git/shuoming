...
在最新版的编译脚本的自定义设置文件（diy-part.sh）里面，可以直接编译了，不需要以上步骤那么麻烦了

export Enable_IPV6_function="1"   # export Enable_IPV6_function="0" ，改成 export Enable_IPV6_function="1" 就搞定
...
在最新版的编译脚本的自定义设置文件（diy-part.sh）里面，可以直接编译了，不需要以上步骤了，开启自定义后，编译好固件，安装完毕后自动关闭那些不需要的IPV6

export Enable_IPV4_function="1"   # export Enable_IPV4_function="0" ，改成 export Enable_IPV4_function="1" 就搞定
